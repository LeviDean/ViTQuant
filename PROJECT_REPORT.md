...
- `mixed_precision_scored()`：混合精度权衡扫描的**任务无关核心**——按敏感度排名保护最敏感的 K 个 block（保留 fp32）、其余量化，扫 K 得到分数 vs 压缩率曲线。同样由 `measure()` 回调提供分数、`score_key` 参数决定行里分数字段名（分类 `top1`、SAM `iou`），所以分类的 `mixed_precision_sweep()` 与 SAM 的 `mixed_precision_sweep_sam()` 都是薄封装。平均权重位宽按各组真实参数量加权，`_weight_params_per_group()` 的分组键与 `_fq_groups()` 严格对齐（都基于量化器名）。
...
- `run_sam.py`：SAM 研究流水线——模拟量化 vision encoder，评估 fp32 vs 量化 mask 的自一致性 IoU，逐块（IoU）敏感度扫描（可跳过，`--skip-sensitivity`），混合精度权衡扫描（可跳过，`--skip-mixed-precision`；依赖敏感度排名），产出 `results.json` 和 `report.md`（IoU 表 + 理论压缩表 + 逐块敏感度表 + 混合精度权衡表）
