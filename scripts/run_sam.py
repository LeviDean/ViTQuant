...
def main() -> None:
    ...
    # model.family selects the SAM generation: "sam" (SAM1, SamModel) or
    # "sam3" (SAM3 tracker, Sam3TrackerModel). Everything downstream is
    # family-agnostic — both share the vision_encoder/prompt_encoder/
    # mask_decoder layout and the iou_scores/pred_masks output fields. SAM3's
    # tracker API requires explicit foreground labels with point prompts.
    family = cfg["model"].get("family", "sam")
    load_model = {"sam": load_sam_model, "sam3": load_sam3_model}[family]
    with_labels = family == "sam3"

    print(f"Loading checkpoint {cfg['model']['checkpoint']} ({family}) ...")
    fp32_model, processor = load_model(cfg["model"]["name"], cfg["model"]["checkpoint"])
