...
和同一批校准/评估样本上进行，保证组间可比（SAM 覆盖 4.1/4.2/4.3/4.4/4.7；消融 4.5 目前是分类
专属）：
...
敏感度排名本身只是中间结论；这一步把它**变成可选型的量化方案**。策略是"**保护最敏感的 K 个
block**"：把排名前 K 的 block 保留 fp32，其余全部量化到配置的低位宽（如 W4），K 从 0（均匀低
位宽）扫到 N（全 fp32），每个 K 真实评估一次，产出一条**精度 vs 压缩率**的权衡曲线。

| K | 保留 fp32 的 block | 实测 top-1 | 平均权重位宽 | 理论压缩比 |
|---|---|---|---|---|
| 0 | （无，均匀低位宽） | … | 4.00 | 8.00x |
| 1 | 最敏感的 1 个 | … | … | … |
| N | 全部（= fp32 基线） | = fp32 | 32.00 | 1.00x |

关键设计约束(与 §4.3 的 caveat 一致)：表里每一行的 top-1 都是**真实评估**得到的，**不是**把
单块敏感度相加预测的——量化误差跨 block 非线性叠加，敏感度排名只用来决定"**先保护谁**"（方案
怎么生成），不用来预测最终精度。平均权重位宽按各 block 的真实参数量加权（保护的 block 算
32-bit，量化的算低位宽），压缩比只统计可量化的权重（激活不落盘）。

这一步复用了逐 block 的量化开关（`set_quantizing` 解耦后的能力）和 §4.3 已算出的排名，几乎不增加
额外校准成本，只多跑 K 组评估。它直接回答 NPU 选型问题："这颗 NPU 如果只能均匀低位宽，精度够
不够？如果不够，保护哪几个 block、以多大的压缩代价能救回来？"这个实验只在低位宽（如 W4A8）下
才有意义——W8A8 下各 block 都不敏感，曲线基本是平的。

**SAM 版（`mixed_precision_sweep_sam`）**：同一套扫描核心（`mixed_precision_scored`），把"分数"从
top-1 换成自一致性 IoU——保护最敏感的 K 层（保留 fp32）、其余量化，扫 K 得到 IoU vs 压缩率曲线
（K=全部时量化模型复现 fp32 的 mask，IoU=1.0）。分类与 SAM 共用核心，差异只在评估函数与分数字段名
（分类 `top1`、SAM `iou`），是"泛化 + 薄封装"的又一处应用。平均权重位宽按各组真实参数量加权——为此
`_weight_params_per_group` 的分组键与 `_fq_groups` 严格对齐（都基于量化器名），避免 SAM 深层命名下
参数漏算。

### 4.5 量化方案消融（Ablation Matrix）
...
### 4.6 W4A8 专项实验（`deit_tiny_w4a8.yaml`）
...
### 4.7 定性可视化（`qualitative.py` / `qualitative_sam.py`）
...
- SAM：fp32 mask（半透明填充）与量化 mask（细边界线）叠加画在原图上（含点提示位置），按 IoU
  从低到高排序、低于阈值（0.9）的标红，用于人工核验"自一致性 IoU 掉得最多的样本，掉的是哪种
  类型的分割错误"。网格 prompt 模式下（`prompt_grid > 1`）每个点分配一种颜色——同色的填充与
  边界线属于同一个 prompt，一张图即可看到全图各处对象的量化前后对比。

这一步不产生新的量化数字，是对 4.2 定量结果的可解释性补充——数字说"平均掉了多少"，图看"具体
错在哪"。

### 4.8 高级 PTQ 算法对比（MSE 截断 / SmoothQuant / AdaRound）

在「observer 标定 + 最近舍入」的朴素 PTQ 之上，框架实现了三个正交、可叠加的后训练量化算法，
作为新的实验维度（配置开关，见 USER_GUIDE §3）：

- **MSE 最优截断**（`observer: mse`）：对数间隔网格搜索截断比例 α，选使 quantize-dequantize
  MSE 最小的 clip 范围，逐通道独立搜索。打「min/max 被离群值撑大」。
- **SmoothQuant**（`smoothquant.enabled`）：校准前的逐输入通道等价变换
  `x@Wᵀ == (x/s)@(s·W)ᵀ`，`s = amax|x|^α / amax|W|^(1−α)`，把激活离群值迁进权重让
  per-channel 权重量化吸收。打「激活个别通道幅值过大」。
- **AdaRound**（`adaround.enabled`）：校准后逐层学习每个权重「向上还是向下舍入」
  （rectified-sigmoid 软舍入 + β 退火正则，优化层输出重构 MSE，无标签），学完固化为
  FakeQuantize 的 `{0,1}` rounding offset——quantizing 开关语义不变，敏感度/混合精度
  实验对 AdaRound 后的模型零改动可用；若学出的舍入劣于最近舍入则自动回退。
  打「4-bit 下最近舍入误差主导」。

实验协议：同一份权重/数据/评估集，逐个叠加算法作对比行。实测基准（本地 MPS）：
deit_tiny W4A8 top-1 掉点 朴素 1.76% → MSE 1.32% → AdaRound 单独 **0.25%**；
SAM ViT-B W4A8 自一致 IoU（8 图 4×4 网格冒烟）朴素 0.566 → MSE 0.675 →
MSE+SmoothQuant+AdaRound（200 iters）**0.705**（GPU 上加大 iters 预期更高）。
报告头部注明启用了哪些算法，保证任意两份报告不会在口径不明的情况下被直接对比。
...
- **SAM 数据**：与分类共享同一套 Imagenette 图像源，校准集/评估集用不同随机种子采样，保证
  两者不重叠。评估默认为每张图撒 `prompt_grid`×`prompt_grid` 的均匀网格点提示（默认 4×4，
  `prompt_grid: 1` 退化为单个图像中心点），IoU 对每个（图像, 点, mask 假设）三元组等权平均——
  网格衡量的是量化对**全图各处对象**分割的影响，而不只是中心物体。被量化的 vision encoder
  每张图只前向一次，多点仅重复很轻的 prompt encoder / mask decoder，评估成本几乎不变；校准
  始终用单中心点（vision encoder 的激活与 prompt 点无关，网格不改变校准统计量）。
- **评估指标**：
...
每次实验（`run_classification.py` 或 `run_sam.py`）落盘到 `outputs/<model>/`：

- `results.json`：结构化数据（各变体的 top-1/top-5 或 IoU、位宽、设备信息），供后续脚本或
  报表二次处理。
- `report.md`：人类可读报告，含精度对比表、理论压缩表、敏感度排名表、**混合精度权衡表**、
  消融矩阵表（SAM 为 IoU 表 + 理论压缩表）。
- `qualitative_grid.png` / `qualitative_sam_grid.png`：§4.7 的可视化网格，附带
